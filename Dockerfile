...
## Start from an existing image with Miniconda installed FROM continuumio/miniconda3
FROM continuumio/anaconda3

MAINTAINER Keith Rumbaua
...
# RUN apt-get -y install build-essential python-cffi libcairo2 libpango-1.0.0 libpangocairo-1.0.0 libgdk-pixbuf2.0.0 libffi-dev shared-mine-info
...
# Run "uwsgi". uWSGI is a Web Server Gateway Interfarce (WSGI) server implementation that is typically used to run Python web applications
